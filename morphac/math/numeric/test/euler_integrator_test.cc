...
TEST_F(EulerIntegratorTest, TrivialStep) {
  ...
  auto derivative1 =
      euler_integrator.Step(State({1, 1, 1}, {}), ControlInput({0, 0}), 0.5);

  // Derivative is zero when the time step is zero.
  auto derivative2 =
      euler_integrator.Step(State(3, 0), ControlInput({1, 1}), 0.);
...
TEST_F(EulerIntegratorTest, Step) {
  ...
  auto updated_state =
      euler_integrator_->Step(State({0, 0, 0}, {}), control_input1, 0.1);
  ASSERT_TRUE(updated_state == State({0.005, 0, 0}, {}));

  // Moving backwards.
  updated_state =
      euler_integrator_->Step(State({0, 0, 0}, {}), control_input2, 0.1);
  ASSERT_TRUE(updated_state == State({-0.005, 0, 0}, {}));

  // Turning in place to the left.
  // 0.5 rad/s for 0.1 seconds. (Angular velocity so only theta changes).
  updated_state =
      euler_integrator_->Step(State({0, 0, 0}, {}), control_input3, 0.1);
  ASSERT_TRUE(updated_state == State({0, 0, 0.05}, {}));

  // Turning in place to the right.
  updated_state =
      euler_integrator_->Step(State({0, 0, 0}, {}), control_input4, 0.1);
  ASSERT_TRUE(updated_state == State({0, 0, -0.05}, {}));

  // Straight line facing a 45 degree angle.
  updated_state = euler_integrator_->Step(State({1., 2., M_PI / 4}, {}),
                                          control_input1, 0.1);
...
TEST_F(EulerIntegratorTest, Integrate) {
  ...
  // Integrating forward.
  auto updated_state =
      euler_integrator_->Integrate(State(3, 0), control_input1, 10, 0.01);
  ASSERT_TRUE(updated_state == State({0.5, 0, 0}, {}));

  // Backward.
  updated_state =
      euler_integrator_->Integrate(State(3, 0), control_input2, 10, 0.01);
  ASSERT_TRUE(updated_state == State({-0.5, 0, 0}, {}));
  ...
  updated_state =
      euler_integrator_->Integrate(State(3, 0), control_input3, M_PI, 0.01);
  ...
  updated_state =
      euler_integrator_->Integrate(State(3, 0), control_input4, 3 * M_PI, 0.01);
  ...
  updated_state = euler_integrator_->Integrate(State({0, 0, M_PI / 2}, {}),
                                               ControlInput({1, 2}),
                                               (2 * M_PI) / 0.5, 0.01);
  ASSERT_TRUE(updated_state == State({0, 0, M_PI / 2}, {}));
}
