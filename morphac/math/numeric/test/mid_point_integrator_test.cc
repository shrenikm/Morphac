...
TEST_F(MidPointIntegratorTest, TrivialStep) {
  // Testing trivial step values with the DiffdriveModel.
  DiffdriveModel diffdrive_model{1.2, 2.5};
  MidPointIntegrator mid_point_integrator{diffdrive_model};

  // Derivative is zero when the wheel velocities are zero.
  auto derivative1 = mid_point_integrator.Step(State({1, 1, 1}, {}),
                                               ControlInput({0, 0}), 0.5);

  // Derivative is zero when the time step is zero.
  auto derivative2 =
      mid_point_integrator.Step(State(3, 0), ControlInput({1, 1}), 0.);
...
TEST_F(MidPointIntegratorTest, Step) {
  ...
  auto updated_state =
      mid_point_integrator_->Step(State({0, 0, 0}, {}), control_input1, 0.1);
  ASSERT_TRUE(updated_state == State({0.005, 0, 0}, {}));

  // Moving backwards.
  updated_state =
      mid_point_integrator_->Step(State({0, 0, 0}, {}), control_input2, 0.1);
  ASSERT_TRUE(updated_state == State({-0.005, 0, 0}, {}));

  // Turning in place to the left.
  // 0.5 rad/s for 0.1 seconds. (Angular velocity so only theta changes).
  updated_state =
      mid_point_integrator_->Step(State({0, 0, 0}, {}), control_input3, 0.1);
  ASSERT_TRUE(updated_state == State({0, 0, 0.05}, {}));

  // Turning in place to the right.
  updated_state =
      mid_point_integrator_->Step(State({0, 0, 0}, {}), control_input4, 0.1);
  ASSERT_TRUE(updated_state == State({0, 0, -0.05}, {}));

  // Straight line facing a 45 degree angle.
  updated_state = mid_point_integrator_->Step(State({1., 2., M_PI / 4}, {}),
                                              control_input1, 0.1);
...
TEST_F(MidPointIntegratorTest, Integrate) {
  // Linear velocity = 0.05
  // Angular velocity = 0
  ControlInput control_input1({0.5, 0.5});
  // Linear velocity = -0.05
  ...
  // Integrating forward.
  auto updated_state =
      mid_point_integrator_->Integrate(State(3, 0), control_input1, 10, 0.01);
  ASSERT_TRUE(updated_state == State({0.5, 0, 0}, {}));

  // Backward.
  updated_state =
      mid_point_integrator_->Integrate(State(3, 0), control_input2, 10, 0.01);
  ASSERT_TRUE(updated_state == State({-0.5, 0, 0}, {}));
  ...
  updated_state =
      mid_point_integrator_->Integrate(State(3, 0), control_input3, M_PI, 0.01);
  ...
  updated_state = mid_point_integrator_->Integrate(State(3, 0), control_input4,
                                                   3 * M_PI, 0.01);
  ...
  updated_state = mid_point_integrator_->Integrate(State({0, 0, M_PI / 2}, {}),
                                                   ControlInput({1, 2}),
                                                   (2 * M_PI) / 0.5, 0.01);
  ASSERT_TRUE(updated_state == State({0, 0, M_PI / 2}, {}));
}

TEST_F(MidPointIntegratorTest, OrderOfIntegration) {
  ...
  // As it is a second order integrator, the absolute differences between the
  // computed and actual values must be of the order dt^2.
  for (int i = 1; i <= 3; ++i) {
