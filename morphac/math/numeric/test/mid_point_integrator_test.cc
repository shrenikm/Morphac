...
TEST_F(MidPointIntegratorTest, TrivialStep) {
  ...
  auto derivative1 =
      mid_point_integrator.Step(State({1, 1, 1}, {}), Input({0, 0}), 0.5);
...
TEST_F(MidPointIntegratorTest, Step) {
  ...
  auto updated_state =
      mid_point_integrator_->Step(State({0, 0, 0}, {}), input1, 0.1);
  ASSERT_TRUE(updated_state == State({0.005, 0, 0}, {}));

  // Moving backwards.
  updated_state =
      mid_point_integrator_->Step(State({0, 0, 0}, {}), input2, 0.1);
  ASSERT_TRUE(updated_state == State({-0.005, 0, 0}, {}));

  // Turning in place to the left.
  // 0.5 rad/s for 0.1 seconds. (Angular velocity so only theta changes).
  updated_state =
      mid_point_integrator_->Step(State({0, 0, 0}, {}), input3, 0.1);
  ASSERT_TRUE(updated_state == State({0, 0, 0.05}, {}));

  // Turning in place to the right.
  updated_state =
      mid_point_integrator_->Step(State({0, 0, 0}, {}), input4, 0.1);
  ASSERT_TRUE(updated_state == State({0, 0, -0.05}, {}));

  // Straight line facing a 45 degree angle.
  updated_state =
      mid_point_integrator_->Step(State({1., 2., M_PI / 4}, {}), input1, 0.1);
  ...
  updated_state =
      mid_point_integrator_->Step(State({1., 2., M_PI / 4}, {}), input2, 0.1);
  ASSERT_TRUE(updated_state == State({1 - 0.005 * cos(M_PI / 4),
                                      2 - 0.005 * sin(M_PI / 4), M_PI / 4},
                                     {}));
}
...
TEST_F(MidPointIntegratorTest, Integrate) {
  ...
  // Integrating forward.
  auto updated_state =
      mid_point_integrator_->Integrate(State(3, 0), input1, 10, 0.01);
  ASSERT_TRUE(updated_state == State({0.5, 0, 0}, {}));

  // Backward.
  updated_state =
      mid_point_integrator_->Integrate(State(3, 0), input2, 10, 0.01);
  ASSERT_TRUE(updated_state == State({-0.5, 0, 0}, {}));
  ...
  updated_state =
      mid_point_integrator_->Integrate(State(3, 0), input3, M_PI, 0.01);
  ...
  updated_state =
      mid_point_integrator_->Integrate(State(3, 0), input4, 3 * M_PI, 0.01);
  ...
  updated_state = mid_point_integrator_->Integrate(
