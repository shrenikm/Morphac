...
def generate_shapes():
    ...
    arc = ArcShape(
        start_angle=np.pi / 4, end_angle=np.pi / 2, radius=2.0, center=[-1.0, 2.0]
    )

    circle = CircleShape(radius=2.0, center=[-1.0, 2.0])

    rectangle = RectangleShape(
        size_x=3.0, size_y=1.5, angle=np.pi / 4, center=[-1.0, 2.0]
    )

    rounded_rectangle = RoundedRectangleShape(
        size_x=3.0, size_y=1.5, angle=np.pi / 4, radius=0.5, center=[-1.0, 2.0]
    )

    triangle = TriangleShape(base=3.0, height=1.5, angle=np.pi / 4, center=[-1.0, 2.0])
...
def test_default_center_arc_shape():
    assert np.allclose(ArcShape(0.0, np.pi, 1.0).center, [0, 0])


def test_invalid_arc_shape():
    # Invalid radius.
    with pytest.raises(ValueError):
        _ = ArcShape(0.0, np.pi / 2, -0.1)
    # Invalid start and end angles (equal).
    with pytest.raises(ValueError):
        _ = ArcShape(np.pi / 2, np.pi / 2, 1.0)
...
def test_default_circle_center_shape():
    assert np.allclose(CircleShape(1.0).center, [0, 0])
...
def test_default_rectangle_center_shape():
    assert np.allclose(RectangleShape(1.0, 1.0, 0.0).center, [0, 0])


def test_invalid_rectangle_shape():
    # Invalid size_x.
    with pytest.raises(ValueError):
        _ = RectangleShape(-0.1, 1.0, 0.0)
    # Invalid size_y.
    with pytest.raises(ValueError):
        _ = RectangleShape(1.0, -0.1, 0.0)
...
def test_default_rounded_rectangle_center_shape():
    assert np.allclose(RoundedRectangleShape(1.0, 1.0, 0.0, 0.1).center, [0, 0])


def test_invalid_rounded_rectangle_shape():
    # Invalid size_x.
    with pytest.raises(ValueError):
        _ = RoundedRectangleShape(-0.1, 1.0, 0.0, 0.1)
    # Invalid size_y.
    with pytest.raises(ValueError):
        _ = RoundedRectangleShape(1.0, -0.1, 0.0, 0.1)
    # Invalid radius.
    with pytest.raises(ValueError):
        _ = RoundedRectangleShape(1.0, 1.0, 0.0, -0.1)
    # The radius must also be smaller than half of the sizes.
    with pytest.raises(ValueError):
        _ = RoundedRectangleShape(3.0, 2.0, 0.0, 1.1)
    with pytest.raises(ValueError):
        _ = RoundedRectangleShape(2.0, 3.0, 0.0, 1.1)
...
def test_default_triangle_center_shape():
    assert np.allclose(TriangleShape(1.0, 1.0, 0.0).center, [0, 0])


def test_invalid_triangle_shape():
    # Invalid base.
    with pytest.raises(ValueError):
        _ = TriangleShape(-0.1, 1.0, 0.0)
    # Invalid height.
    with pytest.raises(ValueError):
        _ = TriangleShape(1.0, -0.1, 0.0)
