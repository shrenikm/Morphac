...
TEST_F(StateTest, CopyConstructor) {
  ...
  VectorXd state_data = VectorXd::Random(10);
  State state2(state_data.head(6), state_data.tail(4));
  State state3(state2);

  ASSERT_TRUE(state1.get_state_data().isApprox(VectorXd::Zero(5)));
  ASSERT_TRUE(state3.get_state_data().isApprox(state_data));
}
...
TEST_F(StateTest, ConstState) {
  ...
  ASSERT_TRUE(state.get_pose().get_pose_data().isApprox(VectorXd::Zero(3)));
  ASSERT_TRUE(
      state.get_velocity().get_velocity_data().isApprox(VectorXd::Zero(3)));
  ...
  // and Velocity.
  const_cast<State&>(state).get_pose().set_pose_data(VectorXd::Ones(3));
  const_cast<State&>(state).get_velocity().set_velocity_data(
      VectorXd::Ones(3));
  ASSERT_TRUE(state.get_pose().get_pose_data().isApprox(VectorXd::Ones(3)));
  ASSERT_TRUE(
      state.get_velocity().get_velocity_data().isApprox(VectorXd::Ones(3)));
}
...
TEST_F(StateTest, GetPose) {
  ...
  pose1.set_pose_data(VectorXd::Ones(3));
  ASSERT_TRUE(pose1.get_pose_data().isApprox(VectorXd::Ones(3)));
  ASSERT_TRUE(state1_->get_pose_data().isApprox(VectorXd::Ones(3)));

  Pose& pose2 = state2_->get_pose();
  pose2.set_pose_data(VectorXd::Ones(4));
  ASSERT_TRUE(pose2.get_pose_data().isApprox(VectorXd::Ones(4)));
  ASSERT_TRUE(state2_->get_pose_data().isApprox(VectorXd::Ones(4)));

  Pose& pose3 = state3_->get_pose();
  pose3.set_pose_data(VectorXd::Ones(2));
  ASSERT_TRUE(pose3.get_pose_data().isApprox(VectorXd::Ones(2)));
  ASSERT_TRUE(state3_->get_pose_data().isApprox(VectorXd::Ones(2)));

  Pose& pose4 = state4_->get_pose();
  pose4.set_pose_data(VectorXd::Ones(1));
  ASSERT_TRUE(pose4.get_pose_data().isApprox(VectorXd::Ones(1)));
...
TEST_F(StateTest, GetVelocity) {
  ...
  velocity1.set_velocity_data(VectorXd::Ones(2));
  ASSERT_TRUE(velocity1.get_velocity_data().isApprox(VectorXd::Ones(2)));
  ASSERT_TRUE(state1_->get_velocity_data().isApprox(VectorXd::Ones(2)));

  Velocity& velocity2 = state2_->get_velocity();
  velocity2.set_velocity_data(VectorXd::Ones(2));
  ASSERT_TRUE(velocity2.get_velocity_data().isApprox(VectorXd::Ones(2)));
  ASSERT_TRUE(state2_->get_velocity_data().isApprox(VectorXd::Ones(2)));

  Velocity& velocity3 = state3_->get_velocity();
  velocity3.set_velocity_data(VectorXd::Ones(3));
  ASSERT_TRUE(velocity3.get_velocity_data().isApprox(VectorXd::Ones(3)));
  ASSERT_TRUE(state3_->get_velocity_data().isApprox(VectorXd::Ones(3)));

  Velocity& velocity4 = state4_->get_velocity();
  velocity4.set_velocity_data(VectorXd::Ones(5));
  ASSERT_TRUE(velocity4.get_velocity_data().isApprox(VectorXd::Ones(5)));
...
TEST_F(StateTest, GetStateData) {
  ASSERT_TRUE(state1_->get_state_data().isApprox(VectorXd::Zero(5)));
  ASSERT_TRUE(state2_->get_state_data().isApprox(VectorXd::Zero(6)));
  ASSERT_TRUE(state3_->get_state_data().isApprox(VectorXd::Zero(5)));
  ASSERT_TRUE(state4_->get_state_data().isApprox(VectorXd::Zero(6)));

  // Arbitrary State.
  VectorXd pose_data = VectorXd::Random(7);
  VectorXd velocity_data = VectorXd::Random(6);
  VectorXd state_data(13);
  state_data << pose_data, velocity_data;
  State state(pose_data, velocity_data);

  ASSERT_TRUE(state.get_state_data().isApprox(state_data));

  // For an empty state, accessing the state data should throw an error.
  ASSERT_THROW(State(0, 0).get_state_data(), std::logic_error);
}
...
TEST_F(StateTest, GetStateAt) {
  ...
  for (int i = 0; i < state.get_size(); ++i) {
    ASSERT_EQ(state(i), state_data(i));
  }
...
TEST_F(StateTest, SetPoseData) {
  ...
  ASSERT_THROW(State(0, 3).set_pose_data(VectorXd::Zero(1)),
               std::logic_error);
  ASSERT_THROW(State(0, 3).set_pose_data(VectorXd::Zero(0)),
               std::logic_error);
}
...
TEST_F(StateTest, SetStateData) {
  // Make sure that when the state is set, it sets both pose and velocity.
  state1_->set_state_data(VectorXd::Ones(5));
  ASSERT_TRUE(state1_->get_pose_data().isApprox(VectorXd::Ones(3)));
  ASSERT_TRUE(state1_->get_velocity_data().isApprox(VectorXd::Ones(2)));
  ASSERT_TRUE(state1_->get_state_data().isApprox(VectorXd::Ones(5)));

  state2_->set_state_data(VectorXd::Ones(6));
  ASSERT_TRUE(state2_->get_pose_data().isApprox(VectorXd::Ones(4)));
  ASSERT_TRUE(state2_->get_velocity_data().isApprox(VectorXd::Ones(2)));
  ASSERT_TRUE(state2_->get_state_data().isApprox(VectorXd::Ones(6)));

  state3_->set_state_data(VectorXd::Ones(5));
  ASSERT_TRUE(state3_->get_pose_data().isApprox(VectorXd::Ones(2)));
  ASSERT_TRUE(state3_->get_velocity_data().isApprox(VectorXd::Ones(3)));
  ASSERT_TRUE(state3_->get_state_data().isApprox(VectorXd::Ones(5)));

  state4_->set_state_data(VectorXd::Ones(6));
  ASSERT_TRUE(state4_->get_pose_data().isApprox(VectorXd::Ones(1)));
  ASSERT_TRUE(state4_->get_velocity_data().isApprox(VectorXd::Ones(5)));
  ASSERT_TRUE(state4_->get_state_data().isApprox(VectorXd::Ones(6)));

  // Aribitrary state.
  VectorXd pose_data(5), velocity_data(5), state_data(10);
  pose_data << 1, -2, 3, -4, 5.5;
  velocity_data << -1, 2, -3, 4, -5.5;
  state_data << pose_data, velocity_data;

  State state(5, 5);
  state.set_state_data({1, -2, 3, -4, 5.5, -1, 2, -3, 4, -5.5});
  ASSERT_TRUE(state.get_pose_data().isApprox(pose_data));
  ASSERT_TRUE(state.get_velocity_data().isApprox(velocity_data));
  ASSERT_TRUE(state.get_state_data().isApprox(state_data));

  // Invalid set.
  ASSERT_THROW(state1_->set_state_data(VectorXd::Zero(4)),
               std::invalid_argument);
  ASSERT_THROW(state1_->set_state_data(VectorXd::Zero(6)),
               std::invalid_argument);

  ASSERT_THROW(state2_->set_state_data(VectorXd::Zero(5)),
               std::invalid_argument);
  ASSERT_THROW(state2_->set_velocity_data(VectorXd::Zero(7)),
               std::invalid_argument);

  ASSERT_THROW(state3_->set_state_data(VectorXd::Zero(4)),
               std::invalid_argument);
  ASSERT_THROW(state3_->set_state_data(VectorXd::Zero(6)),
               std::invalid_argument);

  ASSERT_THROW(state4_->set_state_data(VectorXd::Zero(5)),
               std::invalid_argument);
  ASSERT_THROW(state4_->set_state_data(VectorXd::Zero(7)),
               std::invalid_argument);

  // Setting a state data to an empty state must throw an error.
  ASSERT_THROW(State(0, 0).set_state_data(VectorXd::Zero(1)),
               std::logic_error);
  ASSERT_THROW(State(0, 0).set_state_data(VectorXd::Zero(0)),
               std::logic_error);
}
...
TEST_F(StateTest, Addition) {
  ...
  ASSERT_TRUE(state1.get_state_data().isApprox(s1));
  ...
  state1 += state2;
  ASSERT_TRUE(state1.get_state_data().isApprox(d1));

  State state3 = state1 + state2;
  ASSERT_TRUE(state3.get_state_data().isApprox(d2));

  State state4 = state2 + state1;
  ASSERT_TRUE(state3.get_state_data().isApprox(state4.get_state_data()));
...
TEST_F(StateTest, Subtraction) {
  ...
  State state4 = state2 - state1;
  ASSERT_TRUE(
      state3.get_state_data().isApprox(-1 * state4.get_state_data()));
...
TEST_F(StateTest, Multiplication) {
  ...
  state1 = state1 * 1.0;
  ASSERT_TRUE(state1.get_state_data().isApprox(s1));
  ASSERT_TRUE((0.0 * state1).get_state_data().isApprox(VectorXd::Zero(5)));
  ...
  ASSERT_TRUE(state3.get_state_data().isApprox(d2));
  ASSERT_TRUE(state4.get_state_data().isApprox(d2));
}
...
TEST_F(StateTest, PartialMultiplication) {
  ...
  ASSERT_TRUE((State({1, 2, 3}, {}) * 1.0).get_state_data().isApprox(res1));

  ASSERT_TRUE(
      (State({}, {1, 2, 3}) * 1.0).get_velocity_data().isApprox(res1));
  ASSERT_TRUE((State({}, {1, 2, 3}) * 1.0).get_state_data().isApprox(res1));
}
